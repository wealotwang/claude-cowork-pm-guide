...
Release Gate 指标：
- 高危类（01-05）Recall：32%（7/22）
- 中危类（06-09）Precision：53%（10/19）
- 低危误报率：11%（1/9）

Diagnostic 指标：
- Overall Exact Action Accuracy：50%
- Overall Exact Category Accuracy：38%
- Critical Exact Category Recall：27%
- Medium Recall：53%
...
这说明：
- 系统只是把 08 塌缩成了另一个同样输出 `Warning` 的类别
- 不能因为动作相同，就误认为规则真的做完了

### 3. 当前差距主要不是模型厂商，而是能力覆盖范围
