...
Day05 设计、Day06 接入真实模型跑通验证的最小 demo。只覆盖 4 类风险子集：
...
```bash
# 1. Day06 推荐直接设置你自己的 DeepSeek API key（这一步必须你自己做，不会被本项目存储或上传）
...
不设置 LLM key 也能跑：05类的正则部分不需要 key，会正常工作；01/07/10类的case会被清楚地标记为"跳过"而不是报错崩溃，报告里也会说明有多少条因为没有 key 没测。
...
Day06 更准确的目标不是“再造一批数据”，而是：
- 先接入一个你真实可用的模型（当前优先 DeepSeek）
- 用现有 `eval_cases.json` 跑出第一版真实基线分数
- 看失败 case 集中在哪些规则
- 至少做 1 轮“改规则前后对比”的迭代

这是评测驱动方法论最终要在作品集里体现的证据。
