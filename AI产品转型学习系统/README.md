...
- Module 2 完成：把`checker.py`/评测脚本从`03-学习成果/demo/`迁移到`05-产品原型/规则执行/`，并把`server.py`升级为统一后端，网页上新增"规则执行"tab——现在打开 [05-产品原型/](./05-产品原型/)（`python3 server.py`）就能在浏览器里选数据集（MVP子集/CSV50/自定义路径）、一键跑评测、看指标和失败详情，结果自动存到`命中结果/`。用mock LLM验证过完整链路没问题，真实API key跑CSV50/300 case还需要用户自己验证一次。

当前下一步：
- 合规工作台还有两个模块没做：命中结果（有产物但没独立UI）、分析报告
- 用真实API key在网页上跑一次CSV50/300 case，验证Module 2的一键执行在大样本下表现正常（目前只用mock数据验证过链路，还没验证过真实LLM在大样本下的耗时/稳定性）
- 还没有用大样本（CSV50 / 300 case）验证 Day07 这轮改动（去regex+两个规则修复+重试逻辑）的整体效果，目前只验证了MVP子集30条
